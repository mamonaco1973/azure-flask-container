...
**This video series serves as a complementary guide to the [Simple Azure Containers](https://youtu.be/eogMQjbBvTo) video**, where we explored deploying a Python-based microservice using the Flask framework using containers in Azure. In this new series, we will build upon the original project by configuring **GitHub Actions** to automate the build, test, and deployment processes entirely within the GitHub ecosystem.
...
### 1. **Create a New GitHub Repository from a Template Repository**
   - Navigate to the source repository in a browser.
      ...
      ```bash
        https://github.com/mamonaco1973/azure-flask-container
      ```
...
### 4. **Examine and Push the Backend Files to the Repository**
   - Check to see that `01-acr/01-acr-backend.tf` was created. The content should look something like this:

     ```hcl
      terraform {
         backend "azurerm" {
         resource_group_name   = "terraform-backend-az2vdb"
         storage_account_name  = "tfstateaz2vdb"
         container_name        = "tfstate"
         key                  = "01-acr/terraform.tfstate.json"
         }
      }
     ```
   - Check to see that `03-containerapp/03-containerapp-backend.tf` was created. The content should look something like this:
